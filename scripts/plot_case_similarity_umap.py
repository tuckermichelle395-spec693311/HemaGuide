...
def main() -> None:
    ...
    for group, color in (("leukemia", COLORS["leukemia"]), ("lymphoma", COLORS["lymphoma"])):
        indices = [index for index, row in enumerate(history_rows) if row["group"] == group]
        if not indices:
            continue
        points = history_coords[indices]
        ax.scatter(points[:, 0], points[:, 1], s=34, c=color, alpha=0.78, linewidths=0.35,
                   edgecolors="white", label=f"{'Leukemia' if group == 'leukemia' else 'Lymphoma'} history (n={len(indices)})")
        hull = convex_hull(points)
        if len(hull) >= 3:
            closed = np.vstack([hull, hull[0]])
            ax.plot(closed[:, 0], closed[:, 1], color=color, linewidth=1.1, linestyle=(0, (5, 4)), alpha=0.6)
    unknown_indices = [index for index, row in enumerate(history_rows) if row["group"] == "unknown"]
    if unknown_indices:
        points = history_coords[unknown_indices]
        ax.scatter(points[:, 0], points[:, 1], s=34, c=COLORS["unknown"], alpha=0.7, linewidths=0.35,
                   edgecolors="white", label=f"Unclassified history (n={len(unknown_indices)})")

    for query, coordinate in zip(queries, query_coords):
        color = COLORS.get(query["group"], COLORS["unknown"])
        ax.scatter([coordinate[0]], [coordinate[1]], s=180, marker="s", c=[color], edgecolors="#1D2939",
        ...
        for rank, (case_id, score) in enumerate(query_rankings[query["case_id"]], start=1):
            index = next(i for i, row in enumerate(history_rows) if row["case_id"] == case_id)
            point = history_coords[index]
            ax.annotate(str(rank), xy=point, xytext=(5, 5), textcoords="offset points", fontsize=9,
                        color="#25364D", fontweight="bold", zorder=7,
                        bbox={"boxstyle": "circle,pad=0.18", "fc": "#D9E7F3", "ec": "#25364D", "lw": 0.8})

    ax.set_title("Clinical case similarity search", fontsize=17, pad=16, color="#25364D")
    ax.text(0.5, 1.01, f"{method_name} projection of {len(history_rows)} history cases and {len(queries)} query case(s); Top-{args.top_k} ranked in original embedding space",
